...
-- Maximum distance achievable during the above TimeScale. Should be a
-- rough estimate, but should always be pretty conservative.
DistanceScale = 1000
...
-- Number of training sets to iterate over per frame/update.
TrainingBatchSize = 100

-- Maximum Root Mean Square error allowed before considering training done
-- Multiply this by DistanceScale to get an idea of how far off the
-- distance formula will be for a given time.
-- Too small and training may never complete. Too large and the predictions
-- will be off by quite a bit.
TrainingMaxRMS = .04

-- If the number of training cylces exceeds this number, the neural network
-- will be re-randomized to try again.
TrainingMaxEpochs = 3000

-- Maximum RMS before switching to neural net for aim point prediction.
DummyPredictionRMS = .25
